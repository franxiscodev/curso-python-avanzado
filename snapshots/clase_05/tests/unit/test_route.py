...
async def test_get_route_returns_expected_fields(mock_httpx_route: AsyncMock) -> None:
    ...
    result = await get_route(VALENCIA, DESTINATION, "cycling-regular", "fake-key")
...
async def test_get_route_preserves_profile(mock_httpx_route: AsyncMock) -> None:
    ...
    result = await get_route(VALENCIA, DESTINATION, "driving-car", "fake-key")
...
async def test_get_route_raises_on_bad_key(mocker) -> None:
    ...
    with pytest.raises(httpx.HTTPStatusError):
        await get_route(VALENCIA, DESTINATION, "cycling-regular", "key-invalida")
...
async def test_get_route_accepts_valid_profiles(
    ...
    result = await get_route(VALENCIA, DESTINATION, profile, "fake-key")
